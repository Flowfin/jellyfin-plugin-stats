...
- one `.cdx.json` file, the bill of materials for what the archive was built from
...
release job runs, and the bill of materials is checked for by name in the release job
itself, so a release short of one of the five files is not a state this route can
reach.

The bill of materials is a CycloneDX document listing what the plugin was built
from, generated from the tagged commit by a job of its own and named after the
archive it describes, so a person holding both a year later can still tell that the
two are a pair. It is generated by the same pinned generator that
`.github/workflows/package.yml` runs on every push, so the document attached to a
release and the one produced on a branch are the same document rather than two.
The archive itself is unchanged by this: nothing is added inside it, and a catalog
generator that selects assets by their suffix does not see the new file.

Releases before `0.1.0.0-stable` carry four assets and no bill of materials. That
version's tag is spent and a shipped release is not re-cut, so the list above is
what a release carries from the next tag onward rather than a description of every
release this repository has.
